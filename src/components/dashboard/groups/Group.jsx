...
export default function Students() {
  ...
  useEffect(() => {
    const groupResults = STUDENTS.filter((student) => student.class_name.toLowerCase().includes(groupName.toLowerCase()))
    setStudentList(groupResults)
  }, [])

  function searchName(query) {
    const results = studentList.filter((student) => student.name.toLowerCase().includes(query.toLowerCase()))
    ...
    <div className='bg-gray-100'>
      <div className='m-6 py-6 px-8 bg-white rounded-xl'>
        <div className='flex items-center'>
          <div>
            <p className='text-2xl font-bold'>{`Students from ${groupName}`}</p>
